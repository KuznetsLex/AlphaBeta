...
//проверка написания
        ...
        btnClassify.setOnClickListener {
            ...
            counta += 1
            if (txtOutput.text != ""){
                if ("G" in lettera.substringBefore(',')||"g" in lettera.substringBefore(',')) {
        ...
        propis.setImageResource(R.drawable.bukva_g)
    }
        ...
        when (requestCode) {
            REQ_CODE -> {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if ((a == "G") or (a == "g") or (a == "джи")) {
                        var resID = getResources().getIdentifier("good", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    } else{
                        var resID = getResources().getIdentifier("g_again", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    }
                }

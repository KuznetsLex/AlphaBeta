...
//проверка написания
        ...
        btnClassify.setOnClickListener {
            ...
            counta += 1
            if (txtOutput.text != ""){
                if ("J" in lettera.substringBefore(',')||"j" in lettera.substringBefore(',')) {
                ...
                val randomIntent = Intent(this, J::class.java)
        ...
        propis.setImageResource(R.drawable.bukva_j)
    }
        ...
        when (requestCode) {
            REQ_CODE -> {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if ((a == "J") or (a == "j") or (a == "джей") ) {
                        var resID = getResources().getIdentifier("good", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    } else{
                        var resID = getResources().getIdentifier("j_again", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    }
                }

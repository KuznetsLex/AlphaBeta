...
//проверка написания
        ...
        btnClassify.setOnClickListener {
            ...
            counta += 1
            if (txtOutput.text != ""){
                if ("K" in lettera.substringBefore(',')||"k" in lettera.substringBefore(',')) {
                ...
                val randomIntent = Intent(this, K::class.java)
        ...
        propis.setImageResource(R.drawable.bukva_k)
    }
        ...
        when (requestCode) {
            REQ_CODE -> {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if ((a == "K") or (a == "k") or (a == "кей") or (a == "О$'кей") ) {
                        var resID = getResources().getIdentifier("good", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    } else{
                        var resID = getResources().getIdentifier("k_again", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    }
                }

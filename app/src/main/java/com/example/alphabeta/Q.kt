...
//проверка написания
        ...
        btnClassify.setOnClickListener {
            ...
            counta += 1
            if (txtOutput.text != ""){
                if ("Q" in lettera.substringBefore(',')||"q" in lettera.substringBefore(',')) {
                ...
                val randomIntent = Intent(this, Q::class.java)
        ...
        propis.setImageResource(R.drawable.bukva_q)
    }
        ...
        when (requestCode) {
            REQ_CODE -> {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if ((a == "Q") or (a == "q") or (a == "кю") or (a == "кью")) {
                        var resID = getResources().getIdentifier("good", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    } else{
                        var resID = getResources().getIdentifier("q_again", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    }
                }

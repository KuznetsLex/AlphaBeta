...
//проверка написания
        ...
        btnClassify.setOnClickListener {
            ...
            counta += 1
            if (txtOutput.text != ""){
                if ("U" in lettera.substringBefore(',')||"u" in lettera.substringBefore(',')) {
                ...
                val randomIntent = Intent(this, U::class.java)
        ...
        propis.setImageResource(R.drawable.bukva_u)
    }
        ...
        when (requestCode) {
            REQ_CODE -> {
                if (resultCode == RESULT_OK && null != data) {
                    ...
                    if ((a == "U") or (a == "u") or (a == "ю")) {
                        var resID = getResources().getIdentifier("good", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    } else{
                        var resID = getResources().getIdentifier("u_again", "raw", getPackageName())
                        val mediaPlayer = MediaPlayer.create(this, resID)
                        mediaPlayer.start()
                    }
                }
